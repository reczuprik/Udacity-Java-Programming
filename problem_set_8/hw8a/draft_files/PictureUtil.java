    ...
    public class PictureUtil
    {
        ...
        public static Picture grayAndFlipLeftToRight( Picture pic)
        {
            ...
            int[][] grayPicture = new int[pic.getHeight()][pic.getWidth()];
            grayPicture=pic.getGrayLevels();
            for (int column =0;column<pic.getWidth()/2;column++ )
            {
    
                for (int row =0;row<pic.getHeight();row++ )
                {
                    int temp=grayPicture[row][column];
                    grayPicture[row][column]=grayPicture[row][pic.getWidth()-1-column];
                    grayPicture[row][pic.getWidth()-1-column]=temp;
            ...
            Picture graypic= new Picture(grayPicture); 
            return graypic;
        }
        ...
        public static Picture grayAndRotate90( Picture pic)
        {
            ...
            int[][] grayRotatedPicture = new int[pic.getWidth()][pic.getHeight()];
            for (int column =0; column<pic.getWidth(); column++ )
            {
    
                for (int row =0; row<pic.getHeight(); row++ )
                {
                    //[0][0] -> [0][numberOfColums - 1]
                grayRotatedPicture[column][pic.getHeight()-row-1]=grayPicture[row][column];
                 
            }
        }
        Picture graypic= new Picture(grayRotatedPicture); 
        return graypic; //just so draft will compile. You will change it in the final
    }
